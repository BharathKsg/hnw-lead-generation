...
.hero-eyebrow {
  ...
  letter-spacing: 0.3em; color: var(--gold); text-transform: uppercase; margin-bottom: 16px;
}
.hero-title {
  font-family: 'Cormorant Garamond', serif;
  font-size: clamp(42px, 5vw, 68px); font-weight: 300; line-height: 1.05;
  color: var(--text); margin-bottom: 12px; letter-spacing: -0.02em;
}
.hero-title em { font-style: italic; color: var(--gold-light); }
.hero-sub {
  font-family: 'DM Mono', monospace; font-size: 11px;
  color: var(--text-muted); letter-spacing: 0.08em; margin-top: 8px;
}
...
[data-testid="stTextInput"] input {
  ...
  padding: 16px 20px !important; letter-spacing: 0.02em;
...
[data-testid="stTextInput"] input::placeholder { color: var(--text-muted) !important; font-style: italic; }
...
[data-testid="stButton"] > button {
  ...
  cursor: pointer !important; transition: all 0.2s !important; width: 100%;
}
...
.pipeline-wrap { padding: 40px 64px; background: var(--ink); }
.pipeline-title {
  font-family: 'DM Mono', monospace; font-size: 9px; letter-spacing: 0.3em;
  color: var(--gold-dim); text-transform: uppercase; margin-bottom: 24px;
}
.pipeline-steps { display: flex; align-items: center; margin-bottom: 32px; flex-wrap: wrap; gap: 8px; }
.step-pill {
  font-family: 'DM Mono', monospace; font-size: 9px; letter-spacing: 0.15em;
  text-transform: uppercase; padding: 6px 14px; border-radius: 2px; border: 1px solid transparent;
}
...
.log-console {
  ...
  padding: 20px; height: 200px; overflow-y: auto;
...
.stat-cell { flex: 1; background: var(--surface); padding: 20px 24px; text-align: center; }
.stat-value {
...
.results-header {
  ...
  margin-bottom: 32px; padding-top: 40px; border-top: 1px solid var(--border);
}
.results-title {
  font-family: 'Cormorant Garamond', serif; font-size: 28px;
  font-weight: 300; color: var(--text); letter-spacing: -0.01em;
}
.results-count { font-family: 'DM Mono', monospace; font-size: 10px; color: var(--text-muted); letter-spacing: 0.15em; }
...
.lead-card:hover { border-color: rgba(201,168,76,0.4); box-shadow: 0 4px 32px rgba(0,0,0,0.4); }
.lead-card-accent { position: absolute; left: 0; top: 0; bottom: 0; width: 3px; }
...
.lead-header {
  ...
  padding: 24px 28px 18px 28px; gap: 16px;
}
.lead-name {
  font-family: 'Cormorant Garamond', serif; font-size: 24px; font-weight: 400;
  color: var(--text); letter-spacing: -0.01em; line-height: 1.1;
}
.lead-title-role { font-family: 'DM Mono', monospace; font-size: 10px; color: var(--text-muted); letter-spacing: 0.1em; margin-top: 4px; }
.lead-badges { display: flex; gap: 6px; flex-wrap: wrap; align-items: flex-start; justify-content: flex-end; }
.badge { font-family: 'DM Mono', monospace; font-size: 8px; font-weight: 500; letter-spacing: 0.15em; text-transform: uppercase; padding: 4px 10px; border-radius: 2px; }
...
.lead-body { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 0; }
.lead-section { padding: 18px 28px; border-right: 1px solid var(--border); }
.lead-section:last-child { border-right: none; }
.section-label { font-family: 'DM Mono', monospace; font-size: 8px; letter-spacing: 0.3em; text-transform: uppercase; color: var(--text-muted); margin-bottom: 10px; }
.section-value { font-family: 'Cormorant Garamond', serif; font-size: 18px; font-weight: 300; color: var(--text); }
.section-value.gold { color: var(--gold-light); }
.section-sub { font-family: 'DM Mono', monospace; font-size: 9px; color: var(--text-muted); margin-top: 3px; letter-spacing: 0.05em; }

.lead-known-for {
  padding: 14px 28px 20px; font-family: 'Cormorant Garamond', serif;
...
.footer-chip { font-family: 'DM Mono', monospace; font-size: 9px; letter-spacing: 0.1em; color: var(--text-muted); }
.footer-chip span { color: var(--text-dim); }

.score-bar-wrap { margin-top: 8px; }
.score-bar-track { height: 3px; background: var(--surface3); border-radius: 2px; overflow: hidden; width: 100%; }
.score-bar-fill  { height: 100%; border-radius: 2px; }

.empty-state { text-align: center; padding: 80px 40px; color: var(--text-muted); }
.empty-icon  { font-size: 48px; margin-bottom: 16px; opacity: 0.4; }
.empty-text  { font-family: 'Cormorant Garamond', serif; font-size: 20px; font-style: italic; color: var(--text-muted); }
...
def fmt_net_worth(value, currency="INR"):
    if not value:
        return "—"
    try:
        value = float(value)
        ...
        return str(value)
    if currency == "INR":
        if value >= 1_00_00_00_000: return f"₹{value/1_00_00_00_000:.1f}K Cr"
        if value >= 1_00_00_000:    return f"₹{value/1_00_00_000:.0f} Cr"
        if value >= 1_00_000:       return f"₹{value/1_00_000:.0f} L"
        return f"₹{value:,.0f}"
    sym = {"USD": "$", "EUR": "€", "GBP": "£"}.get(currency, "")
    if value >= 1_000_000_000: return f"{sym}{value/1_000_000_000:.1f}B"
    if value >= 1_000_000:     return f"{sym}{value/1_000_000:.1f}M"
...
def status_accent(s):
    return {"HOT": "hot", "WARM": "warm", "COLD": "cold"}.get(s, "none")

def status_badge(s):
    return {"HOT": "badge-hot", "WARM": "badge-warm", "COLD": "badge-cold"}.get(s, "badge-score")
...
def render_lead_card(lead, idx):
    ...
    nw_raw    = lead.get("net_worth")
    nw_cur    = lead.get("net_worth_currency", "INR")
    nw_src    = lead.get("net_worth_source", "ESTIMATED")
    show_nw   = nw_raw is not None and nw_raw != 0 and str(nw_raw).strip() != ""
    nw_val    = fmt_net_worth(nw_raw, nw_cur) if show_nw else None
    ...
    """ if show_nw else f"""
        <div class="lead-section">
          <div class="section-label">HNI Score</div>
          <div class="section-value gold">{score} / 100</div>
          <div class="score-bar-wrap">
            <div class="score-bar-track">
              <div class="score-bar-fill" style="width:{score}%; background:{score_color(score)};"></div>
            </div>
    ...
    """
    known_html  = f'<div class="lead-known-for">{known_for}</div>' if known_for else ""
    source_html = (
        f'<span class="footer-chip">🔗 <a href="{source_url}" target="_blank" '
        f'style="color:var(--gold-dim);text-decoration:none;">'
        f'{source_url[:55]}{"…" if len(source_url) > 55 else ""}</a></span>'
    ) if source_url else ""

    st.markdown(f"""
    <div class="lead-card">
      <div class="lead-card-accent accent-{status_accent(status)}"></div>
      <div class="lead-header">
        ...
        <div class="lead-badges">
          <span class="badge {status_badge(status)}">{status}</span>
...
# ─────────────────────────────────────────────────────────────────────────────
# Pipeline launcher — subprocess fixes macOS Streamlit DNS timeout
#
# Problem: macOS restricts getaddrinfo() in non-main OS threads.
#   Streamlit's threading.Thread inherits this → DNS times out for any
#   network call (Tavily, Azure, scraping) made inside a thread.
#
# Fix: spawn a child subprocess via sys.executable -c <script>.
#   The child gets its own full OS process and network stack.
#   IPC is done through two temp files:
#     • log_path   — JSONL lines written by the child as it runs
#     • result_path — final JSON array of leads written on completion
#   A lightweight tail thread reads log_path every 0.5 s and pushes
#   entries into the Queue so the UI polls exactly as before.
# ─────────────────────────────────────────────────────────────────────────────

def launch_pipeline(city: str, log_queue: Queue):
    app_dir = str(Path(__file__).parent.resolve())

    # temp files
    log_fd,    log_path    = tempfile.mkstemp(suffix=".log",  prefix="hnw_log_")
    result_fd, result_path = tempfile.mkstemp(suffix=".json", prefix="hnw_res_")
    os.close(log_fd)
    os.close(result_fd)

    # child script (injected strings use repr() for safety)
    runner = f"""
...
def _w(lvl, msg):
    with open(LOG_PATH, "a", encoding="utf-8") as f:
        f.write(json.dumps({{"l": lvl, "m": str(msg)[:300]}}) + "\\n")
        f.flush()

class _FH(logging.Handler):
    def emit(self, record):
        msg = self.format(record)
        if record.levelname == "ERROR":                     lvl = "error"
        elif record.levelname == "WARNING":                 lvl = "warn"
        elif "\\u2713" in msg or "success" in msg.lower(): lvl = "good"
        elif "\\u2717" in msg or "fail" in msg.lower():    lvl = "warn"
        else:                                               lvl = "info"
        _w(lvl, msg)
...
try:
    _w("info", "Initialising pipeline for {city}\\u2026")
    from pipeline.orchestrator import HNWPipeline
    pipeline = HNWPipeline(city={repr(city)})
    leads = pipeline.run()
    _w("good", f"\\u2713 Complete \\u2014 {{len(leads)}} qualified leads found")
    with open(RESULT_PATH, "w", encoding="utf-8") as f:
        json.dump(leads, f, ensure_ascii=False)
    _w("__done__", str(len(leads)))
except Exception as exc:
    import traceback
    _w("error", f"Pipeline error: {{exc}}")
    _w("error", traceback.format_exc()[-400:])
...
"""
    ...
    def _tail():
        ...
        while True:
            ...
            try:
                ...
                for raw in lines[seen:]:
                    ...
                    try:
                        ...
                        lvl, msg = entry.get("l", "info"), entry.get("m", "")
                        if lvl == "__done__":
                            try:
            ...
            # process finished without __done__?
            if proc.poll() is not None:
                time.sleep(1)
                # one final read
                try:
                    with open(log_path, "r", encoding="utf-8") as f:
                        for raw in f.readlines()[seen:]:
                            raw = raw.strip()
                            if not raw: continue
                            try:
                                entry = json.loads(raw)
                                if entry.get("l") == "__done__":
                                    try:
                                    ...
                                    log_queue.put(("__done__", leads))
                                    return
                                log_queue.put((entry.get("l","info"), entry.get("m","")))
                            except: pass
                except: pass
                log_queue.put(("error", f"Process exited (code {proc.returncode}) — check .env and dependencies"))
...
# Session state
# ─────────────────────────────────────────────────────────────────────────────
for k, v in [("leads", []), ("running", False), ("logs", []), ("done", False), ("log_q", None)]:
...
if search_clicked and city_input and not st.session_state.running:
    ...
    launch_pipeline(city_input.strip(), q)
    st.rerun()
...
if st.session_state.running or (st.session_state.done and not st.session_state.leads):
    ...
    if   "complete" in log_text or "upsert" in log_text: active_step = 5
    elif "dedup" in log_text or "unique leads" in log_text: active_step = 4
    elif "analysis" in log_text or "analysing" in log_text: active_step = 3
    elif "chunk" in log_text: active_step = 2
    elif "scrape" in log_text or "attempting" in log_text: active_step = 1
    else: active_step = 0

    pills = '<div class="pipeline-steps">'
    for i, step in enumerate(STEPS):
        if i < active_step:
            ...
            cls = "step-pending"
        pills += f'<span class="step-pill {cls}">{step}</span>'
        if i < len(STEPS) - 1:
            pills += '<span style="color:var(--text-muted);font-size:10px;padding:0 4px">→</span>'
    pills += "</div>"
    st.markdown(pills, unsafe_allow_html=True)

    log_html = "".join(f'<div class="log-line {lvl}">{msg}</div>' for lvl, msg in st.session_state.logs[-40:])
    st.markdown(
        f'<div class="log-console" id="lb">{log_html}'
        '<script>var b=document.getElementById("lb");if(b)b.scrollTop=b.scrollHeight;</script></div>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if not leads:
        ...
        <div class="results-section">
          <div style="padding-top:40px;">
            <div class="stats-bar">
              <div class="stat-cell"><div class="stat-value">{len(leads)}</div><div class="stat-label">Total Leads</div></div>
              <div class="stat-cell"><div class="stat-value" style="color:var(--hot)">{hot_count}</div><div class="stat-label">Hot Leads</div></div>
              <div class="stat-cell"><div class="stat-value" style="color:var(--warm)">{warm_count}</div><div class="stat-label">Warm Leads</div></div>
              <div class="stat-cell"><div class="stat-value">{avg_score}</div><div class="stat-label">Avg HNI Score</div></div>
            </div>
          </div>
        """, unsafe_allow_html=True)
        ...
        with fcol1:
            status_filter = st.selectbox("Status", ["All", "HOT", "WARM", "COLD"], key="filter_status")
        with fcol2:
            sort_by = st.selectbox("Sort by", ["HNI Score ↓", "Net Worth ↓", "Name A–Z"], key="sort_by")
        with fcol3:
            cats = sorted({l.get("category","").replace("_"," ") for l in leads if l.get("category")})
            cat_filter = st.multiselect("Category", cats, key="filter_cat")
        ...
        if sort_by == "HNI Score ↓":
            filtered.sort(key=lambda l: l.get("overall_hni_score", 0) or 0, reverse=True)
        elif sort_by == "Net Worth ↓":
            filtered.sort(key=lambda l: l.get("net_worth", 0) or 0, reverse=True)
        else:
            filtered.sort(key=lambda l: l.get("full_name", "") or "")
